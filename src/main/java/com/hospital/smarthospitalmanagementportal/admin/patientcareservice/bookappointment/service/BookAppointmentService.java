...
//        return savedId.getId();
            if (ad.getId() == 0) {
                ...
                BookAppointment saved = repo.save(d);
                return saved.getId();
            } else {
                // Update existing record
                BookAppointment existing = repo.findById(ad.getId())
                        .orElseThrow(() -> new RuntimeException("Doctor not found"));

                existing.setAdDoctorName(ad.getAdDoctorName());
                existing.setAdSpecialties(ad.getAdSpecialties());
                existing.setAdDesignation(ad.getAdDesignation());
                existing.setAdExperience(ad.getAdExperience());
                existing.setAdAddress(ad.getAdAddress());
                existing.setAdConfirmPassword(ad.getAdConfirmPassword());
                existing.setAdPassword(ad.getAdPassword());
                existing.setAdMobile(ad.getAdMobile());
                existing.setAdDepartment(ad.getAdDepartment());
                existing.setAdQualification(ad.getAdQualification());
                existing.setAdStatus(ad.getAdStatus());
                existing.setLoginDetails(ad.getLoginDetails());
                existing.setAdUsername(ad.getAdUsername());
                existing.setAdShift(ad.getAdShift());

                BookAppointment updated = repo.save(existing);
...
public void checkStatus() {
    ...
    for (BookAppointment ba : list) {
        ...
        if (ba.getSelectedSlot() == null ||
                ba.getPatientPreferredDate() == null) {
            continue;
        }

        try {
            String startTimeStr = ba.getSelectedSlot()
                    .split("-")[0]
                    .trim()
                    .replaceAll("\\s+", " ");
            ...
            LocalDateTime appointmentDateTime =
                    LocalDateTime.of(
                            ba.getPatientPreferredDate(),
                            slotTime
                    );

            // ✅ today + time + 1 minute logic
            if (appointmentDateTime.plusMinutes(1).isBefore(now)) {
                ...
                saveHistory(
                        ...
                        "Admin",
                        "Admin",
                        "Marked as No Show, Please Book New Appointment.",
                        ""
                );
